...
def get_sigma_schedule(args, device):
    ...
    betas = betas.clamp_(min=1e-8, max=0.999)
    sigmas = betas**0.5
...
def train_syndiff(rank, gpu, args):
    ...
    # Networks performing reverse denoising (diffusive only)
    print("Creating networks...")
    print("Num channels for NCSNpp: ", args.num_channels)  # should be 2
    ...
    for epoch in range(init_epoch, args.num_epoch):
        ...
        for iteration, (x1, x2) in enumerate(data_loader):
            # Enable grads for diffusive discriminators
            ...
            # sample from p(x_0)
            real_data1 = x1.to(device, non_blocking=True)  # domain 1
            real_data2 = x2.to(device, non_blocking=True)  # domain 2
            ...
            # Use the real paired partner as guidance (paired supervision)
            x1_0_predict_diff = gen_diffusive_1(torch.cat((x1_tp1.detach(), real_data2), axis=1), t1, latent_z1)
            x2_0_predict_diff = gen_diffusive_2(torch.cat((x2_tp1.detach(), real_data1), axis=1), t2, latent_z2)
            ...
            # Predict x0 using the real paired source as guidance
            x1_0_predict_diff = gen_diffusive_1(torch.cat((x1_tp1.detach(), real_data2), axis=1), t1, latent_z1)
            x2_0_predict_diff = gen_diffusive_2(torch.cat((x2_tp1.detach(), real_data1), axis=1), t2, latent_z2)
            
            # D outputs for fake samples
            x1_pos_sample = sample_posterior(pos_coeff, x1_0_predict_diff[:, [0], :], x1_tp1, t1)
            x2_pos_sample = sample_posterior(pos_coeff, x2_0_predict_diff[:, [0], :], x2_tp1, t2)
            output1 = disc_diffusive_1(x1_pos_sample, t1, x1_tp1.detach()).view(-1)
            output2 = disc_diffusive_2(x2_pos_sample, t2, x2_tp1.detach()).view(-1)
            ...
            # Paired L1 loss on predicted x0 vs real
            ...
            if iteration % 100 == 0 and rank == 0:
                print('epoch {} iteration {}, G-L1: {:.6f}, G-Adv: {:.6f}, G-Sum: {:.6f}, D Loss: {:.6f}'.format(
                    epoch, iteration, errG_L1.item(), errG_adv.item(), errG.item(), errD.item()))
        ...
        if rank == 0:
            ...
            # concatenate noise and real source contrast for both directions
            x1_t_vis = torch.cat((torch.randn_like(real_data1), real_data2), axis=1)
            fake_sample1 = sample_from_model(pos_coeff, gen_diffusive_1, args.num_timesteps, x1_t_vis, T, args)
            ...
            x2_t_vis = torch.cat((torch.randn_like(real_data2), real_data1), axis=1)
            fake_sample2 = sample_from_model(pos_coeff, gen_diffusive_2, args.num_timesteps, x2_t_vis, T, args)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--num_channels', type=int, default=2, help='channel of image')  # keep 2 (x_t + source)
    ...
    parser.add_argument('--lambda_l1_loss', type=float, default=0.5, help='weight of L1 loss part of diffusion')
